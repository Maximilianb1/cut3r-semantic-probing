...
> Layout note: this data pipeline now lives under `data_pipeline/` in the
> repository. Packaging (`pyproject.toml`) stays at the repository root, so run
> the install from the **repository root**. Run the `python -m scripts.*`
> commands and any command with a relative `configs/...` path from **inside
> `data_pipeline/`**. Downstream work (for example Stage 1 segmentation
> validation) lives in sibling top-level directories such as
> `segmentation_validation/` and imports this pipeline as the installed
> `src` / `scripts` packages.

Install the project and development tools (from the repository root):
